...
public class PR123mainTreballadors {
    ...
    public void mostrarTreballadors() throws IOFitxerExcepcio {
        ...
        if (treballadorsCSV != null) {
            System.out.println("Dades dels treballadors:");
    ...
    public void modificarTreballador(String id, String columna, String nouValor) throws IOFitxerExcepcio {
        List<String> treballadorsCSV = llegirFitxerCSV(); // Llegir el fitxer cada cop que es modifica
        ...
        // Validar que la columna existeix
        if (UtilsCSV.obtenirPosicioColumna(treballadorsCSV, columna) == -1) {
            throw new IOFitxerExcepcio("Columna no vàlida: " + columna);
        }

        // Actualitzar la línia del treballador
        UtilsCSV.actualitzarLinia(treballadorsCSV, numLinia, columna, nouValor);
        escriureFitxerCSV(treballadorsCSV);  // Escriure els canvis al fitxer després de la modificació
        System.out.println("Dada actualitzada correctament.");
    }
